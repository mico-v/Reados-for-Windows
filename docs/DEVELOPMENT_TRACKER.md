...
- Rust native MSP tests passed: 181 passed, 0 failed.
...
- `ReadOS.Msp.Hosting.Tests` passed: 348 passed, 0 failed, including the real release-DLL adapter operations with no skip (the native-DLL tests skip only when run standalone without `READOS_MSP_NATIVE_DLL`).
- `ReadOS.App.Tests` passed: 411 passed, 0 failed.
- Managed total: 828 passed, 0 failed (69 + 348 + 411 under the full verifier).
...
- **T95 — Upstream-Aligned Windows MSP Core:** in progress. The handle-based read-only NTFS WorkspaceFS, native adapter, static CRT, direct native `ls`/`cat`, bounded canonical-lowercase `pwd`/`echo` adoption, ABI v2 handshake/release gate, zero-behavior-change Rust command registry, and the `native_mixed_workspace_read_v1`, `native_stream_core_v1`, `mutable_workspace_write_v1`, `recoverable_workspace_trash_v1`, pipelines/redirection, model-facing exec sessions, Windows ConPTY/Job Object process backend + UTF-16LE sanitizer, and ConPTY session integration slices are verified. `native_mixed_workspace_read_v1` adds ABI v2 operation 4 (`WORKSPACE_INVOKE`); `native_stream_core_v1` adds bounded byte-stream primitives; `mutable_workspace_write_v1` adds TOCTOU-safe writes; `recoverable_workspace_trash_v1` adds a hidden `.msp/trash`; pipelines/redirection execute shell pipelines, lists, `!`, and file redirections; model-facing exec sessions add `exec_command`/`write_stdin` (ABI v2 operation 5, capabilities `0x3F` with required `0xF`); the Windows ConPTY/Job Object process backend + UTF-16LE sanitizer are verified by Windows-gated integration tests (skip loudly in nested-job sandboxes); ConPTY session integration runs external processes through exec sessions with stdin continuation and kill-on-expiry. The next work is Debian PTY oracle conformance, broader product adoption, and wider conformance.
...
- The root `MSP/` directory is a local Apache-2.0 source-package/reference input, not a ReadOS runtime dependency or package payload.
- Primary usable inputs are `MSP/Spec`, the available `MSP/Conformance` fixtures/reference outputs, and the committed `conformance/msp-upstream/` snapshot.
- The 2026-08-18 package contains a Windows Cargo workspace with 24 crates and two examples. It is not a ReadOS implementation to wrap or copy.
- `MSP/SOURCE-PACKAGE-README.md` excludes Mac/Swift implementation source; this checkout contains only `MSP/Implementations/Swift/Sources/Tools`, so the previously cited Swift MSPCore/MSPShell/MSPPOSIXCore paths are missing local evidence.
- The documented parity inventory runner/report and required release-gate Python dependencies are absent. The versioned [Windows capability manifest](../conformance/msp-upstream/windows-capability-manifest.json) records those gaps as `blocked`; no passing inventory or full upstream release claim may be inferred from the ReadOS six-case snapshot.
...
| --- | --- | --- |
| 0. Reference and license inventory | Source-package workspace, Spec/Conformance inputs, omitted Swift source, source revision, Apache-2.0/NOTICE, and Windows release-tool availability | The versioned capability manifest maps the 26 workspace members and selected capabilities; raw `MSP/` package exclusion is testable; missing inventory/release runners remain explicitly blocked. |
...
- The local source package exposes the Windows Cargo workspace and selected Spec/Conformance inputs, with Apache-2.0 in `MSP/LICENSE` and project notices in `MSP/NOTICE`. The versioned [capability manifest](../conformance/msp-upstream/windows-capability-manifest.json) records the 24 crates/two examples, ReadOS targets, owners, dependencies, evidence commands, and source/document drift.
- Focused inventory validation passes with `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/verify-msp-capability-manifest.ps1`; it validates path claims, workspace-member coverage, license/provenance presence, raw-MSP Git exclusion, and requires the missing inventory/release evidence to remain `blocked`.
...
- Rust output sanitization covers DOS, verbatim, slash, JSON-escaped, file-URL, and percent-encoded workspace-root forms across chunk boundaries; output and directory resources are bounded. Native UTF-16LE sanitization is implemented and exercised by the ConPTY process path; the current process integration is limited to the in-crate allowlisted `msp_pty_test_child`, so this does not establish general external-process or full MSP parity.
...
- Rust fmt/test/clippy/release build, native binary/FFI smoke, 69 Core tests, 348 Hosting tests (including real release-DLL operations without skip), 411 App tests, and the zero-warning/zero-error solution build pass; Rust has 181 tests and the managed total is 828. The registry candidate release DLL SHA256 is `2CFD14246FA963AC284B158903ADC910A782AFEDFEA4EC5F247692BF4613E49A`.
...
- `native_mixed_workspace_read_v1`, `native_stream_core_v1`, `mutable_workspace_write_v1`, `recoverable_workspace_trash_v1`, pipelines/redirection, and the shell-mode model-facing exec-session path are complete. The Windows ConPTY/Job Object backend, UTF-16LE sanitizer, and ABI/managed ConPTY session integration are implemented as a bounded process-mode slice, but the allowlist currently contains only the in-crate `msp_pty_test_child`; it is not broad production process support or full MSP parity. Debian PTY oracle conformance, a reviewed production executable policy, broader product adoption, and wider shell/command conformance remain as later independent gates. The mixed bridge defines capability traits, longest-prefix mount routing/rebasing, opaque callback handles, disposal, cancellation, concurrency, and .NET delegate lifetime without weakening fixed-local-NTFS confinement, ABI v2, sanitization, or managed lifecycle/audit authority. Synchronous native invocation cannot interrupt an already-running call; current cancellation checks only bound the side-effect-free `pwd`/`echo` calls before and after invocation. Therefore T95 remains in progress.

- `native_runtime_capability_records_v1` is implemented as a Hosting Native contract/test slice for future Python, Node, and Git registration. Only verifier-marked bundle identities with matching bundle id/digest, explicit target RID/PE, explicit scratch mode, bundle-only resolution, and network-disabled policy become `Verified`; missing, mismatched, unverified, or unsafe declarations remain truthful `Blocked` records. The evaluator has no launch, filesystem, network, provider, UI, or command-registry side effects, so runtime installation/launch and product adoption remain deferred.
