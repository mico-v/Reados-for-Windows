...
They currently cover MSP parser/runtime behavior, audit records, streaming command events, transcript progress state, artifact provenance manifests, app virtual workspace projections, approval-gated host commands, and chat command persistence. New MSP work should add tests close to the layer being changed:
...
- transcript tests for progress/cancel state that is surfaced in the workbench;
- policy tests for mutating command allow/confirm/deny behavior and approval previews;
- artifact tests when commands create durable outputs, including returned artifact metadata and `/artifacts/*.manifest.json` projections.
