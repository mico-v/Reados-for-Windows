...
Verified on 2026-08-12: 153 Rust tests, 69 Core tests, 318 Hosting tests (including real release-DLL operations), and 411 App tests (798 managed total under the full verifier), plus native binary/FFI verification and a zero-warning/zero-error solution build. The Hosting count includes the real release-DLL operations; the native-DLL tests skip only when run standalone without `READOS_MSP_NATIVE_DLL`. The registry candidate release DLL SHA256 is `2CFD14246FA963AC284B158903ADC910A782AFEDFEA4EC5F247692BF4613E49A`.
