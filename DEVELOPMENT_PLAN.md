...
- Rust `native/msp-core`: 93 passed; native binary and FFI smoke verification passed. The candidate release DLL SHA256 is `2CFD14246FA963AC284B158903ADC910A782AFEDFEA4EC5F247692BF4613E49A`.
...
Completed adoption gate, `native_mixed_workspace_read_v1`: capability-based read-only WorkspaceFS traits, deterministic longest-prefix mount routing/rebasing, and a lifetime-safe reverse-P/Invoke callback bridge to the app-owned virtual workspace are implemented. ABI v2 operation 4 (`WORKSPACE_INVOKE`) carries the opaque `MspNativeWorkspaceHostV1` table and mount topology into Rust; `CallbackReadOnlyWorkspace` serves `ReadOnlyWorkspaceFileSystem` through invoke/free/is_cancelled callbacks with strict bounds, panic containment, cancellation, free-exactly-once ownership, and host-path disclosure rejection; `workspace_invoke.rs` composes a `CompositeReadOnlyWorkspace` from a callback base plus mounts and serializes bounded JSON results. Rust unit tests (93 total), managed adapter tests, and a real release-DLL differential read test prove direct, virtual, and mixed reads; product `ls`/`cat` are not routed through Rust. The verified fixed-local-NTFS backend, VirtualPath, path sanitization, ABI v2 negotiation/ownership, and managed lifecycle/audit authority are preserved. The next slice is `native_stream_core_v1`. Synchronous native invocation still cannot preempt work already in flight, so current cancellation checks remain bounded around side-effect-free `pwd`/`echo` calls.
...
3. **T95 — Upstream-Aligned Windows MSP Core (in progress):** the read-only handle-based NTFS core, sanitizer, stable Hosting adapter, static CRT, bounded canonical-lowercase `pwd`/`echo` product adoption, length-delimited ABI v2 handshake/release gate, zero-behavior-change Rust command registry, and the `native_mixed_workspace_read_v1` callback bridge are complete. Next add a bounded byte-stream core (`native_stream_core_v1`), mutable WorkspaceFS/trash, pipelines, sessions, ConPTY, and wider conformance without moving ReadOS PDF/chat/workflow behavior into Rust.
