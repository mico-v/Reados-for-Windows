(function(exports, d32) {
  ...
  function isSyntheticRootNode(node) {
    var _a;
    return Boolean(((_a = node == null ? void 0 : node.payload) == null ? void 0 : _a.isSyntheticRoot) || typeof (node == null ? void 0 : node.content) === "string" && node.content.includes("km-synthetic-root"));
  }
  ...
  class Markmap {
    constructor(svg, opts) {
      ...
      this.handleClick = (e, d) => {
        if (isSyntheticRootNode(d)) return;
    ...
    async toggleNode(data, recursive = false) {
      var _a, _b;
      if (isSyntheticRootNode(data)) return;
    ...
    async renderData(originData) {
      ...
      const mmLine = mmGMerge.selectAll(childSelector("line")).data(
        (d) => {
          return isSyntheticRootNode(d) ? [] : [d];
        },
        (d) => d.state.key
      );
      ...
      const mmCircle = mmGMerge.selectAll(childSelector("circle")).data(
        (d) => {
          var _a;
          return isSyntheticRootNode(d) ? [] : (((_a = d.children) == null ? void 0 : _a.length) ? [d] : []);
        },
        (d) => d.state.key
      );
      ...
      const links = nodes.flatMap(
        (node) => {
          ...
          return (payload == null ? void 0 : payload.fold) || isSyntheticRootNode(node) ? [] : node.children.map((child) => ({ source: node, target: child }));
        }
      );
      ...
      const lineOverlap = (d) => -Math.min(1, lineWidth(d) / 2);
      this.transition(mmLineMerge).attr("x1", lineOverlap).attr("x2", (d) => d.state.rect.width + 2).attr("stroke", (d) => color(d)).attr("stroke-width", lineWidth);
