...
They currently cover MSP parser/runtime behavior, audit records, structured diagnostics, streaming command events, transcript progress state, durable session records, workflow summaries, artifact provenance manifests, app virtual workspace projections, approval-gated host commands, and chat command persistence. New MSP work should add tests close to the layer being changed:
